...
class UpdatePublicEventPage extends StatefulWidget {
  eventList.Data data;
  UpdatePublicEventPage({Key key,  this.data,}) : super(key: key);


  @override
  _UpdatePublicEventPageState createState() => _UpdatePublicEventPageState();
}

class _UpdatePublicEventPageState extends State<UpdatePublicEventPage> {
  ...
  Widget build(BuildContext context) {
    return user.accountType.compareTo(CommonUtils.ACCOUNT_TYPE_NORMAL_USER) == 0
        ? Container(
            padding: EdgeInsets.symmetric(horizontal: 35),
            child: Center(
              child: Text(
                checkDateIsToday()? "Only account with Business Type and VIP Type can create this event.":"No events Found",
                              ...
                              _selectedBusinessAccount != null
                                  ? Container(
                                      ...
                                      child: ListTile(
                                        ...
                                        title: Text(_selectedBusinessAccount
                                                .firstName +
                                            " " +
                                            _selectedBusinessAccount.lastName),
                                        subtitle: Text(_selectedBusinessAccount
                                                .city +
                                            " (" +
  ...
  Future<GetMyFriendsResponse> getMyFriendsList() async {
    return ApiImplementer.getMyFriendsList(
        accessToken: user.accessToken, search_query: _friendController.text.trim(), user_id: user.userId);
  }
