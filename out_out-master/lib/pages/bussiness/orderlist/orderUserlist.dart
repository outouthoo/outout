...
import 'OrderUserDetailScreen.dart';
class OrderUserList extends StatefulWidget {
  @override
  OrderUserListState createState() => OrderUserListState();
}

class OrderUserListState extends State<OrderUserList> {
  User user;
  SharedPreferences _sharedPreferences;
  bool _isLoading = false;
  OrderUserResponse _response;
  String errorMessage = "";

  List<OrderUserItem> orderList = [];
  ...
  void getBookingForUserList() async {
    ...
    ApiImplementer.getOrderUserList(
  ...
  void _showRatingAppDialog(OrderUserItem item) {
    final _ratingDialog = RatingDialog(
      ...
      onSubmitted: (response) {
        ...
        addRating(item,response.rating,response.comment);
      },
    );
                ...
                ? Expanded(
                  child: ListView.builder(
                      ...
                      itemBuilder: (BuildContext context, int index) {
                        var item = orderList.elementAt(index);
                        var menuitem = "";
                        ...
                        return InkWell(
                          onTap: () async{
                            ...
                            var data = await Navigator.of(context)
                              ...
                              builder: (context) => OrderUserDetailScreen(item),
                            ));
                            getBookingForUserList();
                          },
                          child: Container(
                              ...
                              children: [
                                ...
                                Padding(
                                  ...
                                  child: Row(
                                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                                    children: [
                                      user.accountType == "0" && item.orderStatus == "0"
                                          ? InkWell(
                                              child: Container(
                                                width: 120,
                                                decoration: BoxDecoration(
                                              ...
                                              onTap: () {
                                                changeTableStatusAPICall(3, item.id);
                                              },
                                            )
                                          :
                                      user.accountType == "0" && item.orderStatus == "1"
                                          ? InkWell(
                                              child: Container(
                                                width: 120,
                                                decoration: BoxDecoration(
  ...
  void payOrder(OrderUserItem item) {
    ApiImplementer.payOrder(
            accessToken: user.accessToken, id: item.id)
        .then((value) {
      if (value.errorcode == ApiImplementer.SUCCESS) {
        CommonDialogUtil.showSuccessSnack(context: context, msg: value.msg);
        _showRatingAppDialog(item);
  ...
  void addRating(OrderUserItem item, double rating, String comment,) {
    ApiImplementer.addRating(
            accessToken: user.accessToken, fromUserId: user.userId,toUserId:item.toUserId,rating: rating.toInt().toString(),comments: comment)
        .then((value) {
      if (value.errorcode == ApiImplementer.SUCCESS) {
        CommonDialogUtil.showSuccessSnack(context: context, msg: value.msg);
      } else if (value.errorcode == "2") {
